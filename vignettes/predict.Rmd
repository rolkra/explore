...
We train a random forest model, using `buy` as target (`buy` contains only 0 and 1 values)

```{r message=FALSE, warning=FALSE, fig.width=6, fig.height=4}
train %>% explain_forest(target = buy)
```
...
`prediction_1` (the probability of `buy == 1`)

```{r message=FALSE, warning=FALSE, fig.width=6, fig.height=4}
test %>% explore(prediction_1, target = buy)
```

We can check the predictions by comparing `prediction_1` with real values of buy.
