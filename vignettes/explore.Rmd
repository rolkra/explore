...
Interactive data exploration with one line of code or use a easy to remember set of tidy functions. Introduces two main verbs for data analysis. **describe()** to describe a variable or table, **explore()** to grafically explore a variable or table.

explore package on Github: [https://github.com/rolkra/explore](https://github.com/rolkra/explore)

As the explore-funcitons fits well into the tidyverse, we load the dplyr-package as well.
...
A shiny app is launched, you can inspect individual attributes, explore their relation to a binary target, grow a decision tree or create a fully automated report of all attributes with a few "mouseclicks".
...
iris$is_versicolor <- ifelse(iris$Species == "versicolor", 1, 0)
iris %>% report(output_file = "report.html", target = is_versicolor)
...
Add titel, detailed descriptions and change default filename
...
dwh <- dwh_connect("DWH_DSN", user = "myuser")
